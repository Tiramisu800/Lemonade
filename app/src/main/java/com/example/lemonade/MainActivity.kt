...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        when(lemonadeState){
            SELECT -> {
                //image is clicked in the SELECT state, the state should become SQUEEZE
                lemonadeState = SQUEEZE
                //determine lemonSize by pick()
                val tree: LemonTree = lemonTree
                lemonSize = tree.pick()
                //at start no squeeze, 0 squeeze
                squeezeCount = 0
            }
            SQUEEZE -> {
                squeezeCount++
    ...
    private fun setViewElements() {
        ...
        //Track lemonadeState and put UI accordingly
        when(lemonadeState){
            SELECT -> {
